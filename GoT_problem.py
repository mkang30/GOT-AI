...
class GoTState(GameState):
    def __init__(self, board, player_locs, ptm, prev_cell_type, ww_locs, ww_directions):
        self.board = board
        self.player_locs = player_locs
        self.ptm = ptm
        self.prev_cell_type = prev_cell_type
        self.ww_locs = ww_locs
        self.ww_directions = ww_directions
...
class GoTProblem(AdversarialSearchProblem):
    def __init__(self, board_file_loc, first_player, message_print):
        ...
        self._start_state = GoTState(board, player_locs, first_player, prev_cell_type, ww_locs, ww_directions)
        # game constants
    ...
    def transition(self, state, action):
        ...
        ww_directions = [dir for dir in state.ww_directions]
        ...
        # Enters perm area of current player
            ...
            # The player has to return to the base; only reaching the last step of trail is not 'close'
                ...
                if capture_other_player_bool:
                    ...
                    return GoTState(board, player_locs, next_ptm, prev_cell_type, ww_locs,ww_directions)
    ...
    def transition_runner(self, state, action):
        ...
        if ptm != self.start_player:
            ...
            state = self.move_ww(state.board, state.player_locs, state.ww_locs,state.ww_directions)
        ...
        return GoTState(state.board, state.player_locs, next_ptm, state.prev_cell_type, state.ww_locs, state.ww_directions)

    def move_ww(self, board, player_locs, ww_locs, ww_directions):
        num_wws = len(ww_locs)
        for ww in range(num_wws):

            old_trajectory = ww_directions[ww]
            this_loc = ww_locs[ww]
            ...
            ww_directions[ww] = new_trajectory
    ...
    def get_safe_actions(board, loc, ptm):
        ...
        """
        if ptm == 0:
            unsafe_vals = {CellType.WALL, CellType.ONE_TEMP, CellType.WHITE_WALKER}
        elif ptm == 1:
            unsafe_vals = {CellType.WALL, CellType.TWO_TEMP, CellType.WHITE_WALKER}
        else:
            raise Exception
